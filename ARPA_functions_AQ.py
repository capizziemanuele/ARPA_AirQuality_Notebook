...
def ARPA_sensors_info(client) -> pd.DataFrame:
    ...
    stationsId = "ib47-atvt" # Select aq stations dataset containing positions and information about sensors
...
def req_ARPA_start_end_date_API(client):
    ...
    #AQ sensors dataset id on Open Data Lombardia
    aq_sensor_id = "nicp-bhqi"
    
    #Query min and max dates
    query = """ select MAX(data), MIN(data) limit 9999999999999999"""

    #Get max and min dates from the list
    min_max_dates = client.get(aq_sensor_id, query=query)[0]
...
def req_ARPA_data_API(client, start_date, end_date, sensors_list):
    """
    Function to request data from available AQ sensors in the ARPA API using a query.
    ...
    #Select the Open Data Lombardia AQ sensors dataset
    aq_sensor_id = "nicp-bhqi"
    ...
    time_series = client.get(aq_sensor_id, query=query)
...
def download_extract_csv_from_year(year):
    """
    Function for selecting the correct link for downloading zipped .csv aq data from ARPA sensors and extracting it.

    For older data it is necessary to download this .csv files containing the time series of the aq sensors.

        Parameters:
            year(str): the selected year for downloading the .csv file containing the aq sensors time series
    ...
    switcher = {
        '2022': "https://www.dati.lombardia.it/download/3vy4-yusn/application%2Fx-zip-compressed",
    ...
    if not os.path.exists(year+".csv"):
        ...
        with ZipFile(filename, 'r') as zObject:
            ...
            zObject.extractall()
            os.rename(str(year)+'.csv', 'Aria_'+str(year)+'.csv')
        ...
        print(csv_file+" already exists. It won't be downloaded.")
